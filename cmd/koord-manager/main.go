...
import (
	...
	extclient "Kooperator/pkg/client"
	"Kooperator/pkg/features"
	sloconfig "Kooperator/pkg/slo-controller/config"
	"Kooperator/pkg/slo-controller/nodemetric"
	"Kooperator/pkg/slo-controller/noderesource"
	"Kooperator/pkg/slo-controller/nodeslo"
	utilclient "Kooperator/pkg/util/client"
	utilfeature "Kooperator/pkg/util/feature"
	"Kooperator/pkg/util/fieldindex"
	_ "Kooperator/pkg/util/metrics/leadership"
	"Kooperator/pkg/webhook"
...
func main() {
	...
	flag.StringVar(&leaderElectionNamespace, "leader-election-namespace", "kooperator-system",
	...
	setRestConfig(cfg)
	cfg.UserAgent = "kooperator-manager"

	setupLog.Info("new clientset registry")
	err := extclient.NewRegistry(cfg)
	if err != nil {
		setupLog.Error(err, "unable to init kooperator clientset and informer")
		os.Exit(1)
	}
	...
	mgr, err := ctrl.NewManager(cfg, ctrl.Options{
		...
		LeaderElectionID:           "kooperator-manager",
