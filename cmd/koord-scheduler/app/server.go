...
import (
	...
	schedulerserverconfig "Kooperator/cmd/koord-scheduler/app/config"
	"Kooperator/cmd/koord-scheduler/app/options"
	"Kooperator/pkg/scheduler/eventhandlers"
...
func NewSchedulerCommand(schedulingHooks []frameworkext.SchedulingPhaseHook, registryOptions ...Option) *cobra.Command {
	...
	cmd := &cobra.Command{
		Use: "koord-scheduler",
		Long: `The Kooperator scheduler is a control plane process which assigns
...
func Run(ctx context.Context, cc *schedulerserverconfig.CompletedConfig, sched *scheduler.Scheduler) error {
	...
	klog.V(1).InfoS("Starting Kooperator Scheduler version", "version", version.Get())
	...
	cc.KooperatorSharedInformerFactory.Start(ctx.Done())
	cc.NRTSharedInformerFactory.Start(ctx.Done())

	// Wait for all caches to sync before scheduling.
	cc.InformerFactory.WaitForCacheSync(ctx.Done())
	cc.KooperatorSharedInformerFactory.WaitForCacheSync(ctx.Done())
...
func Setup(ctx context.Context, opts *options.Options, schedulingHooks []frameworkext.SchedulingPhaseHook, outOfTreeRegistryOptions ...Option) (*schedulerserverconfig.CompletedConfig, *scheduler.Scheduler, error) {
	...
	extendedHandle := frameworkext.NewExtendedHandle(
		frameworkext.WithKooperatorClientSet(cc.KooperatorClient),
		frameworkext.WithKooperatorSharedInformerFactory(cc.KooperatorSharedInformerFactory),
