...
这里使用一个 ConfigMap 管理 SLO 配置。该 ConfigMap 被 slo-controller 所使用，它的名字和命名空间可以在 koord-manager 的启
动参数中指定（默认为 `kooperator-system/slo-controller-config`）。它分别包含了以下键值：
...
```yaml
apiVersion: slo.kooperator/v1alpha1
kind: NodeSLO
metadata:
  name: test-node
spec:
...
1. 通过 ConfigMap `kooperator-system/slo-controller-config` 检查当前的 SLO 配置。

```bash
$ kubectl get configmap -n kooperator-system slo-controller-config -o yaml
apiVersion: v1
kind: ConfigMap
metadata:
  name: slo-controller-config
...
2. 编辑 ConfigMap `kooperator-system/slo-controller-config` 来修改 SLO 配置。

```bash
$ kubectl edit configmap -n kooperator-system slo-controller-config
```
...
```bash
$ kubectl get nodeslo.slo.kooperator.sh test-node -o yaml
apiVersion: slo.kooperator.sh/v1alpha1
kind: NodeSLO
metadata:
  name: test-node
spec:
