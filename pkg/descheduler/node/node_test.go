...
import (
	...
	"Kooperator/pkg/descheduler/test"
)
...
func TestReadyNodes(t *testing.T) {
	...
	node2.Status.Conditions = []corev1.NodeCondition{{Type: corev1.NodeMemoryPressure, Status: corev1.ConditionTrue}}
	node3 := test.BuildTestNode("node4", 1000, 2000, 9, nil)
	node3.Status.Conditions = []corev1.NodeCondition{{Type: corev1.NodeNetworkUnavailable, Status: corev1.ConditionTrue}}
	...
	node5.Status.Conditions = []corev1.NodeCondition{{Type: corev1.NodeReady, Status: corev1.ConditionFalse}}
...
func TestIsNodeUnschedulable(t *testing.T) {
	...
	for _, tt := range tests {
		actualUnSchedulable := IsNodeUnschedulable(tt.node)
		if actualUnSchedulable != tt.IsUnSchedulable {
			t.Errorf("Test %#v failed", tt.description)
		}
	}
...
func TestPodFitsCurrentNode(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			node: test.BuildTestNode("node1", 64000, 128*1000*1000*1000, 200, func(node *corev1.Node) {
...
func TestPodFitsAnyOtherNode(t *testing.T) {
	...
	tests := []struct {
		...
		nodes       []*corev1.Node
		success     bool
		podsOnNodes []*corev1.Pod
	}{
		{
			description: "Pod fits another node matching node affinity",
			pod: test.BuildTestPod("p1", 0, 0, nodeNames[2], func(pod *corev1.Pod) {
				pod.Spec.NodeSelector = map[string]string{
					nodeLabelKey: nodeLabelValue,
				}
			}),
			nodes: []*corev1.Node{
				test.BuildTestNode(nodeNames[0], 64000, 128*1000*1000*1000, 200, func(node *corev1.Node) {
					node.ObjectMeta.Labels = map[string]string{
						nodeLabelKey: nodeLabelValue,
					}
				...
				test.BuildTestNode(nodeNames[1], 64000, 128*1000*1000*1000, 200, func(node *corev1.Node) {
					node.ObjectMeta.Labels = map[string]string{
						nodeLabelKey: "no",
					}
		...
		},
		{
			...
			pod: test.BuildTestPod("p1", 2000, 2*1000*1000*1000, nodeNames[2], func(pod *corev1.Pod) {
				pod.Spec.NodeSelector = map[string]string{
					nodeLabelKey: nodeLabelValue,
				}
				pod.Spec.Containers[0].Resources.Requests[corev1.ResourceEphemeralStorage] = *resource.NewQuantity(10*1000*1000*1000, resource.DecimalSI)
			}),
			nodes: []*corev1.Node{
				test.BuildTestNode(nodeNames[0], 64000, 128*1000*1000*1000, 200, func(node *corev1.Node) {
					node.ObjectMeta.Labels = map[string]string{
		...
		},
		{
			...
			podsOnNodes: []*corev1.Pod{
				test.BuildTestPod("test-pod", 12*1000, 20*1000*1000*1000, nodeNames[1], func(pod *corev1.Pod) {
					...
					pod.Spec.Containers[0].Resources.Requests[corev1.ResourceEphemeralStorage] = *resource.NewQuantity(40*1000*1000*1000, resource.DecimalSI)
				}),
			},
			success: true,
		},
		{
			...
			nodes: []*corev1.Node{
				...
				test.BuildTestNode(nodeNames[1], 4000, 8*1000*1000*1000, 12, func(node *corev1.Node) {
					node.ObjectMeta.Labels = map[string]string{
						nodeLabelKey: nodeLabelValue,
					}

					node.Status.Allocatable[corev1.ResourceEphemeralStorage] = *resource.NewQuantity(200*1000*1000*1000, resource.DecimalSI)
				}),
				test.BuildTestNode(nodeNames[2], 0, 0, 0, nil),
			},
			podsOnNodes: []*corev1.Pod{
				test.BuildTestPod("3-core-pod", 3000, 4*1000*1000*1000, nodeNames[1], func(pod *corev1.Pod) {
					pod.ObjectMeta = metav1.ObjectMeta{
						Namespace: "test",
						Labels: map[string]string{
							"test": "true",
		...
		},
		{
			...
			pod: test.BuildTestPod("p1", 2000, 5*1000*1000*1000, nodeNames[2], func(pod *corev1.Pod) {
				pod.Spec.NodeSelector = map[string]string{
					nodeLabelKey: nodeLabelValue,
				}
				pod.Spec.Containers[0].Resources.Requests[corev1.ResourceEphemeralStorage] = *resource.NewQuantity(10*1000*1000*1000, resource.DecimalSI)
			...
			nodes: []*corev1.Node{
				...
				test.BuildTestNode(nodeNames[1], 10*1000, 8*1000*1000*1000, 12, func(node *corev1.Node) {
					node.ObjectMeta.Labels = map[string]string{
						nodeLabelKey: nodeLabelValue,
					}
			...
			podsOnNodes: []*corev1.Pod{
				test.BuildTestPod("4GB-mem-pod", 2000, 4*1000*1000*1000, nodeNames[1], func(pod *corev1.Pod) {
					pod.ObjectMeta = metav1.ObjectMeta{
						Namespace: "test",
						Labels: map[string]string{
							"test": "true",
		...
		},
		{
			...
			pod: test.BuildTestPod("p1", 2000, 4*1000*1000*1000, nodeNames[2], func(pod *corev1.Pod) {
				pod.Spec.NodeSelector = map[string]string{
					nodeLabelKey: nodeLabelValue,
				}
				pod.Spec.Containers[0].Resources.Requests[corev1.ResourceEphemeralStorage] = *resource.NewQuantity(10*1000*1000*1000, resource.DecimalSI)
			...
			nodes: []*corev1.Node{
				...
				test.BuildTestNode(nodeNames[1], 10*1000, 8*1000*1000*1000, 12, func(node *corev1.Node) {
					...
					node.Status.Allocatable[corev1.ResourceEphemeralStorage] = *resource.NewQuantity(20*1000*1000*1000, resource.DecimalSI)
				}),
				test.BuildTestNode(nodeNames[2], 0, 0, 0, nil),
			},
			podsOnNodes: []*corev1.Pod{
				test.BuildTestPod("11GB-storage-pod", 2000, 4*1000*1000*1000, nodeNames[1], func(pod *corev1.Pod) {
					pod.ObjectMeta = metav1.ObjectMeta{
						Namespace: "test",
						Labels: map[string]string{
							"test": "true",
					...
					pod.Spec.Containers[0].Resources.Requests[corev1.ResourceEphemeralStorage] = *resource.NewQuantity(11*1000*1000*1000, resource.DecimalSI)
				}),
			},
			success: false,
		},
		{
			...
			podsOnNodes: []*corev1.Pod{
				test.BuildTestPod("10-custom-resource-pod", 0, 0, nodeNames[1], func(pod *corev1.Pod) {
					pod.ObjectMeta = metav1.ObjectMeta{
						Namespace: "test",
						Labels: map[string]string{
							"test": "true",
		...
		},
		{
			...
			pod: test.BuildTestPod("p1", 1000, 2*1000*1000*1000, nodeNames[2], func(pod *corev1.Pod) {
				pod.Spec.NodeSelector = map[string]string{
					nodeLabelKey: nodeLabelValue,
				}
				pod.Spec.Containers[0].Resources.Requests[corev1.ResourceEphemeralStorage] = *resource.NewQuantity(10*1000*1000*1000, resource.DecimalSI)
			...
			nodes: []*corev1.Node{
				...
				test.BuildTestNode(nodeNames[1], 5000, 8*1000*1000*1000, 12, func(node *corev1.Node) {
					node.ObjectMeta.Labels = map[string]string{
						nodeLabelKey: nodeLabelValue,
					}
			...
			podsOnNodes: []*corev1.Pod{
				test.BuildTestPod("3-core-pod", 2000, 4*1000*1000*1000, nodeNames[1], func(pod *corev1.Pod) {
					pod.ObjectMeta = metav1.ObjectMeta{
						Namespace: "test",
						Labels: map[string]string{
							"test": "true",
...
func createResourceList(cpu int64, memory int64, ephemeralStorage int64) corev1.ResourceList {
	...
	resourceList[corev1.ResourceEphemeralStorage] = *resource.NewQuantity(ephemeralStorage, resource.DecimalSI)
	return resourceList
}
...
func TestPodFitsAnyNode(t *testing.T) {
	tests := []struct {
		...
		{
			name: "Either of the two nodes matches the Pod via affinity and tolerations",
			pod: &corev1.Pod{
			...
			nodes: []*corev1.Node{
				{
					ObjectMeta: metav1.ObjectMeta{
						Name: "test-node-C",
						Labels: map[string]string{
							"test-node-type": "test-node-type-C",
						},
					},
				},
				{
		...
		{
			name: "Neither of the two nodes matches the Pod via affinity and tolerations",
			pod: &corev1.Pod{
				ObjectMeta: metav1.ObjectMeta{
					Namespace: "default",
					Name:      "tet-pod-1",
			...
			nodes: []*corev1.Node{
				...
				},
				{
					ObjectMeta: metav1.ObjectMeta{
						Name: "test-node-D",
						Labels: map[string]string{
							"test-node-type": "test-node-type-D",
						},
					},
				},
			},
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if got := PodFitsAnyNode(getPodsAssignedToNode, tt.pod, tt.nodes); got != tt.want {
