...
func TestListPodsOnANode(t *testing.T) {
	testCases := []struct {
		...
		},
		{
			...
			pods: []*corev1.Pod{
				test.BuildTestPod("pod1", 100, 0, "n1", nil),
				test.BuildTestPod("pod2", 100, 0, "n1", func(pod *corev1.Pod) {
					pod.Labels = map[string]string{"foo": "bar"}
				}),
				test.BuildTestPod("pod3", 100, 0, "n1", func(pod *corev1.Pod) {
		...
		},
		{
			...
			pods: []*corev1.Pod{
				test.BuildTestPod("pod1", 100, 0, "n1", func(pod *corev1.Pod) {
					pod.Namespace = "included-ns-1"
				}),
				test.BuildTestPod("pod2", 100, 0, "n1", func(pod *corev1.Pod) {
					pod.Namespace = "included-ns-2"
				}),
				test.BuildTestPod("pod3", 100, 0, "n1", nil),
				test.BuildTestPod("pod4", 100, 0, "n2", nil),
			},
			node:               test.BuildTestNode("n1", 2000, 3000, 10, nil),
			includedNamespaces: []string{"included-ns-1", "included-ns-2"},
			expectedPodCount:   2,
		},
		{
			...
			node:               test.BuildTestNode("n1", 2000, 3000, 10, nil),
			excludedNamespaces: []string{"included-ns-1", "included-ns-2"},
			expectedPodCount:   1,
		},
		{
			name: "test listing pods with filter func",
			pods: []*corev1.Pod{
				test.BuildTestPod("pod1", 100, 0, "n1", func(pod *corev1.Pod) {
					pod.Namespace = "included-ns-1"
				}),
			...
			filterFunc: func(pod *corev1.Pod) bool {
				return strings.HasPrefix(pod.Namespace, "included")
			},
			expectedPodCount: 2,
		},
	}
	for _, testCase := range testCases {
		t.Run(testCase.name, func(t *testing.T) {
			...
			filter, err := NewOptions().
				WithLabelSelector(testCase.labelSelector).
				WithNamespaces(sets.NewString(testCase.includedNamespaces...)).
				WithoutNamespaces(sets.NewString(testCase.excludedNamespaces...)).
				WithFilter(testCase.filterFunc).
				BuildFilterFunc()
			if err != nil {
...
func TestSortPodsBasedOnPriorityLowToHigh(t *testing.T) {
	...
	p1 := test.BuildTestPod("p1", 400, 0, n1.Name, func(pod *corev1.Pod) {
		test.SetPodPriority(pod, lowPriority)
	})

	// BestEffort
	p2 := test.BuildTestPod("p2", 400, 0, n1.Name, func(pod *corev1.Pod) {
		test.SetPodPriority(pod, highPriority)
		test.MakeBestEffortPod(pod)
	})

	// Burstable
	p3 := test.BuildTestPod("p3", 400, 0, n1.Name, func(pod *corev1.Pod) {
		test.SetPodPriority(pod, highPriority)
		test.MakeBurstablePod(pod)
	})

	// Guaranteed
	p4 := test.BuildTestPod("p4", 400, 100, n1.Name, func(pod *corev1.Pod) {
		test.SetPodPriority(pod, highPriority)
	...
	podList := []*corev1.Pod{p4, p3, p2, p1, p6, p5}
...
func TestSortPodsBasedOnAge(t *testing.T) {
	podList := make([]*corev1.Pod, 9)
	n1 := test.BuildTestNode("n1", 4000, 3000, int64(len(podList)), nil)

	for i := 0; i < len(podList); i++ {
		podList[i] = test.BuildTestPod(fmt.Sprintf("p%d", i), 1, 32, n1.Name, func(pod *corev1.Pod) {
