...
func TestCPUSetIsSubsetOf(t *testing.T) {
	shouldBeSubset := []struct {
		...
		// A CPUSet is a subset of itself
		{NewCPUSet(), NewCPUSet()},
		{NewCPUSet(5), NewCPUSet(5)},
		{NewCPUSet(1, 2, 3, 4, 5), NewCPUSet(1, 2, 3, 4, 5)},

		// Empty CPUSet is a subset of every CPUSet
		{NewCPUSet(), NewCPUSet(5)},
		{NewCPUSet(), NewCPUSet(1, 2, 3, 4, 5)},
...
func TestCPUSetToSlice(t *testing.T) {
	...
	for _, c := range testCases {
		...
		if !reflect.DeepEqual(result, c.expected) {
			t.Fatalf("expected CPUSet as slice to be [%v] (got [%v]), s: [%v]", c.expected, result, c.set)
		}
	}
}
...
func TestCPUSetString(t *testing.T) {
	...
	for _, c := range testCases {
		...
		if result != c.expected {
			t.Fatalf("expected CPUSet as string to be %s (got \"%s\"), s: [%v]", c.expected, result, c.set)
		}
	}
}
...
func TestParse(t *testing.T) {
	...
	for _, c := range testCases {
		...
		if !c.wantError && err != nil {
			t.Fatalf("unexpected error have occurred: %v", err)
		}
		if c.wantError && err == nil {
			t.Fatalf("expected error not to have occurred")
		}
		if !result.Equals(c.expected) {
