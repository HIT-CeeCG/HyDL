...
import (
	...
	schedulingconfig "Kooperator/apis/scheduling/config"
	schedulingconfigv1beta2 "Kooperator/apis/scheduling/config/v1beta2"
...
func newPluginTestSuit(t *testing.T, nodes []*corev1.Node) *pluginTestSuit {
	...
	return &pluginTestSuit{
		Handle:                          fh,
		kooperatorClientSet:             koordClientSet,
		kooperatorSharedInformerFactory: koordSharedInformerFactory,
...
func (p *pluginTestSuit) start() {
	...
	p.kooperatorSharedInformerFactory.Start(ctx.Done())
	p.nrtSharedInformerFactory.Start(ctx.Done())
	p.Handle.SharedInformerFactory().WaitForCacheSync(ctx.Done())
	p.kooperatorSharedInformerFactory.WaitForCacheSync(ctx.Done())
