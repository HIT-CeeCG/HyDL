...
func GetPodMemStatUsageBytes(podCgroupDir string) (int64, error) {
	podMemStatPath := GetPodCgroupMemStatPath(podCgroupDir)
	return readCgroupMemStat(podMemStatPath)
}

func GetContainerMemStatUsageBytes(podCgroupDir string, c *corev1.ContainerStatus) (int64, error) {
	containerMemStatPath, err := GetContainerCgroupMemStatPath(podCgroupDir, c)
	if err != nil {
		return 0, err
	}
	return readCgroupMemStat(containerMemStatPath)
}
