...
// +kubebuilder:rbac:groups=config.kooperator,resources=clustercolocationprofile,verbs=get;list;watch
...
func (h *PodMutatingHandler) doMutateByColocationProfile(ctx context.Context, pod *corev1.Pod, profile *configv1alpha1.ClusterColocationProfile) error {
	...
	if profile.Spec.KooperatorPriority != nil {
		if pod.Labels == nil {
			pod.Labels = make(map[string]string)
		}
		pod.Labels[extension.LabelPodPriority] = fmt.Sprintf("%d", *profile.Spec.KooperatorPriority)
	}
