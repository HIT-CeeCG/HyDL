...
func TestClusterColocationProfileMutatingPod(t *testing.T) {
	...
	pod := &corev1.Pod{
		ObjectMeta: metav1.ObjectMeta{
			...
			Labels: map[string]string{
				"kooperator-colocation-pod": "true",
			},
		},
		Spec: corev1.PodSpec{
	...
	namespaceObj := &corev1.Namespace{
		ObjectMeta: metav1.ObjectMeta{
			Name: "default",
			Labels: map[string]string{
				"enable-kooperator-colocation": "true",
			},
		},
	}
	...
	profile := &configv1alpha1.ClusterColocationProfile{
		...
		Spec: configv1alpha1.ClusterColocationProfileSpec{
			...
			SchedulerName:      "kooperator-scheduler",
			QoSClass:           string(extension.QoSBE),
			PriorityClassName:  "kooperator-batch",
			KooperatorPriority: pointer.Int32(1111),
	...
	err = handler.clusterColocationProfileMutatingPod(context.TODO(), req, pod)
	assert.NoError(err)
